...
if { $target eq "alinx" } {
    puts "Target is ALINX"
    set_property verilog_define {ALINX=1} [get_filesets sources_1]
} elseif { $target eq "puzhi" } {
    puts "Target is PUZHI"
    set_property verilog_define {PUZHI=1} [get_filesets sources_1]
} else {
    puts "ERROR: Unsupported target '$target'"
    exit 1
}
